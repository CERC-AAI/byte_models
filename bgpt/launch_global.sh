...
RELOAD_CONFIG_PATH="/lustre/orion/csc590/scratch/george-adams/bgpt2/byte_models/bgpt/configs/reload_config_110M.yaml"
...
do
  ...
  if [ "$DEPENDENCY" == "none" ]; then
    JOB_SUBMIT_OUTPUT=$(sbatch --job-name=$JOB_NAME --output=${SRC_DIR}/logs/%x-%j.out --time=2:00:00 --partition=batch --nodes=32 launch.sh --train-config-path ${CONFIG_PATH})
  else
    JOB_SUBMIT_OUTPUT=$(sbatch --dependency=afterany:$DEPENDENCY --job-name=$JOB_NAME --output=${SRC_DIR}/logs/%x-%j.out --time=2:00:00 --partition=batch --nodes=32 conditional_launch.sh --train-config-path ${RELOAD_CONFIG_PATH})
  fi
